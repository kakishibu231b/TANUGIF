...
namespace TANUGIF
{
    partial class MainForm
    {
            ...
            else
            {
                ...
                for (int ii = 0; ii < intNodeCount; ii++)
                {
                    if (parentNode.Nodes[ii] == selectedNode)
                    {
                        ...
                        else if (ii == parentNode.Nodes.Count)
                        {
        ...
        private void viewPropertyDialog()
        {
            ...
            if (parentNode == null)
            {
                formProperty.textBoxName.Text = selectedNode.Text;
                ...
                formProperty.numericUpDownFrameCount.Value = selectedNode.GetNodeCount(false);
                formProperty.numericUpDownFrameCount.Minimum = formProperty.numericUpDownFrameCount.Value;

                formProperty.numericUpDownWidth.Enabled = true;
                formProperty.numericUpDownHeight.Enabled = true;
                formProperty.numericUpDownFrameCount.Enabled = true;
            }
            // フレーム
            else if (parentNode.Parent == null)
            {
                ...
                formProperty.numericUpDownFrameCount.Value = parentNode.GetNodeCount(false);
                formProperty.numericUpDownFrameCount.Minimum = formProperty.numericUpDownFrameCount.Value;

                formProperty.numericUpDownWidth.Enabled = false;
                formProperty.numericUpDownHeight.Enabled = false;
                formProperty.numericUpDownFrameCount.Enabled = false;
            }
            // 画像
            else
            {
                ...
                formProperty.numericUpDownWidth.Value = parentNodeTag.Bitmap.Width;
                formProperty.numericUpDownHeight.Value = parentNodeTag.Bitmap.Height;
                formProperty.numericUpDownFrameCount.Value = parentNode.Parent.GetNodeCount(false);
                formProperty.numericUpDownFrameCount.Minimum = formProperty.numericUpDownFrameCount.Value;
            ...
            {
                if(formProperty.numericUpDownFrameCount.Minimum < formProperty.numericUpDownFrameCount.Value)
                {
                    for ( int ii = (int)formProperty.numericUpDownFrameCount.Minimum; ii < formProperty.numericUpDownFrameCount.Value; ++ii)
                    {
                        toolStripButtonAddFrameNode.PerformClick();
                    }
                }

                selectedNode.Text = formProperty.textBoxName.Text;
                ...
                if (parentNode == null)
                {
                    ...
                    foreach (TreeNode frame in selectedNode.Nodes)
                    {
                        foreach (TreeNode image in frame.Nodes)
                        {
                            ...
                            int posX = imageNodeTag.Point.X;
                            int posY = imageNodeTag.Point.Y;
