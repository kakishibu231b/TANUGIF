...
namespace TANUGIF
{
    partial class MainForm
    {
        ...
        private void pictureBoxPreview_Redraw(TreeNode selectedNode, bool bolUpdateDataBrowser = true)
        {
            // 未選択状態は処理なし
        ...
        private void pictureBoxPreview_MouseMove2(object sender, MouseEventArgs e)
        {
            if ((e.Button & MouseButtons.Left) == MouseButtons.Left)
            {
                ...
                // プレビュー再描画
                pictureBoxPreview_Redraw(selectedNode, false);
        ...
        private void changeImagePos(TreeNode selectedNode)
        {
            ...
            if (parentNode == null)
            {
                foreach(TreeNode frameNode in selectedNode.Nodes)
                {
                    changeImagePos(frameNode);
                }
                return;
            }
