...
namespace TANUGIF
{
    public partial class MainForm : Form
    {
        ...
        /// <param name="frameNode"></param>
        /// <param name="srcItem"></param>
        private void addImageNodeToFrame(TreeNode frameNode, string strFilePath, int intFrameNumber, Point point = new Point())
        {
            if (!File.Exists(strFilePath))
            {
                return;
            ...
            TreeViewControl.addImageNode(frameNode, strImageKey, strImageName, image, strFilePath, intFrameNumber, point);
            ...
            if (openFileDialogProject.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (Project project in projectFile.ProjectList)
                {
                    ...
                    foreach (Frame frame in project.FrameList)
                    {
                        ...
                        {
                            addImageNodeToFrame(nodeFrame, frameImage.FilePath, frameImage.FrameNumber, frameImage.Point);
                        }
                    }
                    nodeProject.Expand();
                }
