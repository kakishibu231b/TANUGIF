...
namespace TANUGIF
{
    public partial class MainForm : Form
    {
        ...
        private void listViewLib_DragEnter(object sender, DragEventArgs e)
        {
            ...
            {
                if ( toolStripComboBoxLibFolder.SelectedIndex > 0)
                {
        ...
        {
            // コピー先フォルダ取得
            ComboBoxLibFolderElement comboBoxLibFolderElement = toolStripComboBoxLibFolder.SelectedItem as ComboBoxLibFolderElement;

            // コピー元ファイル取得
            string[] strDropFileNames = (string[])e.Data.GetData(DataFormats.FileDrop, false);

            // 同一ファイル名存在チェック
            bool bolCancel = false;
            foreach (string strDropFileName in strDropFileNames)
            {
                // コピー元ファイル名
                string strFileNameWithoutDir = Path.GetFileName(strDropFileName);

                // コピー先ファイル名フルパス
                string strToPath = Path.Combine(comboBoxLibFolderElement.Path, strFileNameWithoutDir);

                // コピー先フォルダに同一ファイル名が存在するか？
                bool bolFileExist = File.Exists(strToPath);

                // 上書き確認
                if (bolFileExist)
                {
                    DialogResult dialogResult = MessageBox.Show("同一ファイル名がありますが上書きしますか？\n" + strToPath, "ファイルの上書き確認", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            ...
            foreach (string strDropFileName in strDropFileNames)
            {
                ...
                // ファイルコピー
                File.Copy(strDropFileName, strToPath, true);
            }
